...
TGKLSConstrainedProblem::TGKLSConstrainedProblem(EConstrainedProblemType problemType, double fraction,
    ...
    int numConstraints)
    : IConstrainedOptProblem(dim, {}, {}, -1, problemType, fraction, activeConstrNum)
{
    mDimension = dim;
    mProblemIndex = problemIndex;
    mLoBound.assign(mDimension, -1.0);
    mUpBound.assign(mDimension, 1.0);

    // Теперь размер вектора опций зависит от переданного числа
    int totalFunctions = numConstraints + 1;
    options.resize(totalFunctions);
    mFunctions.clear();
    mConstraintIndeces.clear();
    mCriterionIndeces.clear();

    for (int i = 0; i < totalFunctions; i++) {
        // Настраиваем параметры генератора для каждой функции
        options[i].index = (problemIndex + i - 1) % 100 + 1; // Разные индексы для каждой функции
        options[i].isArgSet = 0;
        options[i].mIsGeneratorMemoryAllocated = false;
        options[i].mIsDomainMemeoryAllocated = true;
        options[i].rnd_num = new double[NUM_RND];
        options[i].rand_condition = new double[KK];
        options[i].mFunctionType = functionType;
        options[i].SetFunctionClass(type, mDimension);

        // Генерируем саму функцию
        SetOptions(i);

        // Если это первая функция (i=0) - это критерий
        if (i == 0) {
            ...
            if (options[0].isArgSet == 1) {
                ...
                for (int d = 0; d < mDimension; d++) mOptimumPoint[d] = x[d];
            }
            mOptimumValue = Compute(0, mOptimumPoint);

            mFunctions.push_back({ mDimension, mOptimumPoint, mOptimumValue, {}, 0, -1, true, false, false, true });
            mCriterionIndeces.push_back(0);
        }
        // Остальные - это ограничения
        else {
            vector<double> constrOpt(mDimension);
            if (options[i].isArgSet == 1) {
                double* x = options[i].GKLS_minima.local_min[options[i].GKLS_glob.gm_index[0]];
                for (int d = 0; d < mDimension; d++) constrOpt[d] = x[d];
            }
            mFunctions.push_back({ mDimension, constrOpt, Compute(i, constrOpt), {}, 0, -1, true, false, false, true });
...
void TGKLSConstrainedProblem::SetOptions(int problemIndex)
{
    ...
    // Используем индивидуальный индекс для каждой функции в наборе
    int currentSeed = options[problemIndex].index;

    int err_code = options[problemIndex].GKLS_arg_generate(currentSeed, 0, mDimension, mLoBound, mUpBound);
